 ...
 */
class GenerateDartFromJsonHelper {
    ...
    fun generateJson(backupJsonFileDirPath: String, dartFileDirPath: String, worldCityFilePath: String) {
        ...
        var worldCityFile: File? = null
        if (!worldCityFilePath.isNullOrEmpty()) {
            worldCityFile = File(worldCityFilePath)
        }

        if (jsonFilePathList.isEmpty() && (worldCityFile == null || !worldCityFile.exists())) {
            logE("$backupJsonFileDirPath 和 $worldCityFilePath 目录下面没有找人任何json文件")
            return
        }
        var worldCityJsonInfo: JsonInfo? = null
        if (worldCityFile != null && worldCityFile.exists()) {
            worldCityJsonInfo = collectWorldCityKeyValueInfo(worldCityFilePath)
        }
        ...
        var worldMethodInfoList = mutableListOf<MethodInfo>()
        if (worldCityJsonInfo != null) {
            worldMethodInfoList = collectMethodInfo(mutableListOf(worldCityJsonInfo))
        }
    ...
    private fun generateTranslateManagerDartClass(
        ...
        bufferedWriter.write("import 'dart:ui';")
        ...
        bufferedWriter.newLine();
        bufferedWriter.write("  final List<LocalCallBack> _localCallBackList = [];")
        bufferedWriter.newLine()
        ...
        bufferedWriter.newLine()
        bufferedWriter.newLine()
        bufferedWriter.write("  void addLocalCallBack(LocalCallBack localCallBack) {")
        bufferedWriter.newLine()
        bufferedWriter.write("    if (_localCallBackList.contains(localCallBack)) return;")
        bufferedWriter.newLine()
        bufferedWriter.write("    _localCallBackList.add(localCallBack);")
        bufferedWriter.newLine()
        bufferedWriter.write("  }")
        bufferedWriter.newLine()
        bufferedWriter.newLine()
        bufferedWriter.write("  void remoLocalCallBack(LocalCallBack localCallBack) {")
        bufferedWriter.newLine()
        bufferedWriter.write("    _localCallBackList.remove(localCallBack);")
        bufferedWriter.newLine()
        bufferedWriter.write("  }")
        bufferedWriter.newLine()
        bufferedWriter.newLine()
        ...
        bufferedWriter.newLine()
        bufferedWriter.write("    _notifyLocalChanged(preLocal: _currentLocal, newLocal: locale);")
        bufferedWriter.newLine()
        ...
        if (worldCityMethodInfoList.isNotEmpty()) {
            bufferedWriter.newLine()
            bufferedWriter.write("  String getWorldValue(String worldKey, Locale locale) {")
            bufferedWriter.newLine()
            ...
            for (methodInfo in worldCityMethodInfoList) {
                ...
                bufferedWriter.newLine()
                bufferedWriter.write("        result = _getTranslateByLocale(locale).${methodInfo.key}();")
                bufferedWriter.newLine()
                bufferedWriter.write("        break;")
                bufferedWriter.newLine()
            }
            ...
            bufferedWriter.newLine()
            bufferedWriter.newLine()
            bufferedWriter.write("  BaseTranslate _getTranslateByLocale(Locale locale) {")
            bufferedWriter.newLine()
            bufferedWriter.write("    BaseTranslate translate;")
            bufferedWriter.newLine()
            bufferedWriter.write("    switch (_currentLocal.languageCode.toLowerCase()) {")
            for (itemDartClassInfo in dartClassInfoList) {
                bufferedWriter.newLine()
                bufferedWriter.write("      case \"${itemDartClassInfo.local}\":")
                bufferedWriter.newLine()
                bufferedWriter.write("        translate = ${itemDartClassInfo.className}();")
                bufferedWriter.newLine()
                bufferedWriter.write("        break;")
            }
            bufferedWriter.newLine()
            bufferedWriter.write("      default:")
            bufferedWriter.newLine()
            bufferedWriter.write("        translate = ${defaultTranslate.className}();")
            bufferedWriter.newLine()
            bufferedWriter.write("        break;")
            bufferedWriter.newLine()
            bufferedWriter.write("    }")
            bufferedWriter.newLine()
            bufferedWriter.write("    return translate;")
            bufferedWriter.newLine()
            bufferedWriter.write("  }")
        }
        ...
        bufferedWriter.newLine()
        bufferedWriter.newLine()
        bufferedWriter.write("  void _notifyLocalChanged(")
        bufferedWriter.newLine()
        bufferedWriter.write("      {required Locale preLocal, required Locale newLocal}) {")
        bufferedWriter.newLine()
        bufferedWriter.write("    if (preLocal.languageCode == newLocal.languageCode &&")
        bufferedWriter.newLine()
        bufferedWriter.write("        preLocal.countryCode == newLocal.countryCode) return;")
        bufferedWriter.newLine()
        bufferedWriter.write("    for (var itemCallBack in _localCallBackList) {")
        bufferedWriter.newLine()
        bufferedWriter.write("      itemCallBack.onLocalChanged(preLocal, newLocal);")
        bufferedWriter.newLine()
        bufferedWriter.write("    }")
        bufferedWriter.newLine()
        bufferedWriter.write("  }")
        bufferedWriter.newLine()
        bufferedWriter.newLine()
        bufferedWriter.write("  void release() {")
        bufferedWriter.newLine()
        bufferedWriter.write("    _localCallBackList.clear();")
        bufferedWriter.newLine()
        bufferedWriter.write("    emptyMap.clear();")
        bufferedWriter.newLine()
        bufferedWriter.write("  }")
        bufferedWriter.newLine()
        bufferedWriter.write("}")
        bufferedWriter.newLine()
        bufferedWriter.newLine()
        bufferedWriter.write("abstract class LocalCallBack {")
        bufferedWriter.newLine()
        bufferedWriter.write("  ///语言发生改变")
        bufferedWriter.newLine()
        bufferedWriter.write("  void onLocalChanged(Locale originalLocal, Locale newLocal);")
        bufferedWriter.newLine()
        bufferedWriter.write("}")
        bufferedWriter.newLine()
        ...
        for (index in 1 until jsonInfoList.size) {
            ...
            for (itemMethodInfo in methodInfoList) {
                try {
                    ...
                    e.printStackTrace()
                    logD("itemMethodInfo=$itemMethodInfo")
                }
            }
        }
    ...
    private fun getWorldCityKeyValueInfoFromJson(jsonFilePath: String, valueInfoMap: MutableMap<String, ValueInfo>) {
        try {
            ...
            for (itemMapEntry in jsonMap) {
                val nameKey = itemMapEntry.nameKey
                val nameValue = itemMapEntry.name
                if (!nameKey.isNullOrEmpty() && !nameValue.isNullOrEmpty()) {
                    if (!valueInfoMap.containsKey(nameKey)) {
                        valueInfoMap[nameKey] = ValueInfo(key = nameKey, value = nameValue, argList = mutableListOf())
                    }
                }
                val countryKey = itemMapEntry.nameKey
                val countryValue = itemMapEntry.name
                if (!countryKey.isNullOrEmpty() && !countryValue.isNullOrEmpty()) {
                    if (!valueInfoMap.containsKey(countryKey)) {
                        valueInfoMap[countryKey] =
                            ValueInfo(key = countryKey, value = countryValue, argList = mutableListOf())
                    }
                }
            }
        } catch (e: Exception) {
