...
//            }
            ...
            if (selectedText.startsWith(getTranslateKeyPrefix())) {
                ...
                if (result.isEmpty()) {
                    ...
                    showSearchResult(result = mutableListOf(result))
                }
            } else {
    ...
    private fun showSearchResult(result: List<List<TranslateInfo>>) {
        ...
        val firstTranslateInfo=result.first()
        for (rowIndex in firstTranslateInfo.indices) {
        ...
        for (column in 0 until jbTable.columnCount) {
            jbTable.columnModel.getColumn(column)
                .setCellRenderer({ table, value, isSelected, hasFocus, row, column ->
                    ...
                    jTextArea.rows=lineCountList[row]
                    jTextArea
                })
        }
        jbTable.columnModel.getColumn(0).maxWidth=180
        ...
        scroll.setSize(screenSize.width, screenSize.height-100)
        scroll.add(jbTable)
        jbTable.setSize(screenSize.width, screenSize.height-100)
     ...
     *开始通过Key找翻译
     */
    private fun findTranslateByKey(keyValue: String, event: AnActionEvent): List<TranslateInfo> {
        logD("开始通过key查找翻译信息")
        ...
        val result = mutableListOf<TranslateInfo>()
        result.add(zhTranslateInfo)
